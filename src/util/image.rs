...
// impl draw of set pixel
// then do file of drawing functions

impl ImageBuffer {
    pub fn new(wh: IVec2) -> Self {
        ImageBuffer {
            wh,
            data: vec![0; (wh.x*wh.y*4) as usize],
        }
    }
    pub fn from_bytes(png_bytes: &[u8]) -> Self {
        ...
        ImageBuffer {
            wh: ivec2(info.width as i32, info.height as i32),
            data,
        }
    }
    pub fn get(&self, x: usize, y: usize) -> Vec4 {
        let idx = (y*self.wh.x as usize + x)*4;
        vec4(
    ...
    pub fn fill(&mut self, colour: Vec4) {
        for i in 0..self.wh.x {
            for j in 0..self.wh.y {
                self.set(i as _, j as _, colour);
            }
        }
    }
    pub fn set_square(&mut self, x: usize, y: usize, r: usize, colour: Vec4) {
        for j in (y as isize - r as isize).max(0) as usize..=(y + r).min(self.wh.y as usize - 1) {
            for i in (x as isize - r as isize).max(0) as usize..=(x + r).min(self.wh.x as usize - 1) {
                self.set(i, j, colour);
            }
        }
    }
